...
| Factory | `Docs/ogrenme/13-desen-secim-rehberi.md:527` | KAPALI | Ölçü, üç doğum yolu, tetikleyici, ne kırar, ön koşul ve kod taslağı yazılı; ölçü: `StructureProduction.Produce` somut tipi `out Combatant produced` parametresiyle imzasında taşıyor, `Combatant` ve `Structure` ikisi de `sealed`, üretilen somut tip sayısı bir |
