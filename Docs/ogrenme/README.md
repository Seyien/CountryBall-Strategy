...
| [04-yok-olan-mekanizmalar-unity.md](04-yok-olan-mekanizmalar-unity.md) | Motorun sunduğu ama bu projenin **almadığı** beş mekanizmanın işleyişini altı alanla yazar: arkada ne oluyor, sahibi kim, ölçüsü ne, bu projede nereye düşerdi, ne kırar, alternatifi ne | Tetikleyici koşul **yazmaz** — o `02`'nin işidir. Sürüme bağlanmamış motor iddiası kurmaz |
| [05-yok-olan-mekanizmalar-csharp.md](05-yok-olan-mekanizmalar-csharp.md) | `yield` ve `await` gördüğünde derleyicinin ürettiği **sınıfı** IL'den ölçerek gösterir; dört mekanizmayı (coroutine · `Task` · `Awaitable` · iş parçacığı) ayırır | Motor tarafını anlatmaz (`konular/08`), `foreach` tarafını anlatmaz (`dil/02`), "şimdi ekleyelim" demez |
| [06-ilkeler-ve-kokenleri.md](06-ilkeler-ve-kokenleri.md) | Dokuz ilkenin adını, kökenini, projedeki karşılığını ve iki uzunlukta mülakat cevabını yazar; **ilkeler çatıştığında** hangisinin kazandığını koddan gösterir | Mekanizma anlatmaz — çoğu `Docs/deep/` altında zaten anlatılı, buradan link verilir |
